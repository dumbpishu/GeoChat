...
type MessageInputProps = {
  onSend: (text: string, media?: UploadedMedia[], mentions?: string[], replyTo?: string) => void | Promise<void>;
  onTypingStart: () => void;
  onTypingStop: () => void;
...
export const MessageInput = ({ 
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await onSend(text.trim(), uploadedMedia, mentionIds.length > 0 ? mentionIds : undefined, replyMessage?._id);
      setText("");
      setMedia([]);
      setSelectedMentions([]);
      setShowMentions(false);
  ...
  return (
    <div className="bg-slate-50/50 px-4 py-4">
      {/* Reply preview bar */}
      {replyMessage && (
        <div className="flex items-center gap-2 mb-2 px-3 py-2 bg-white border border-slate-200 rounded-xl shadow-sm">
          <div className="flex flex-col items-start gap-0.5 min-w-0">
            <div className="flex items-center gap-1.5">
              <Reply className="w-3 h-3 text-sky-500" />
              <p className="text-xs font-semibold text-sky-600">
                Replying to {replyMessage.senderId?.name || replyMessage.senderId?.username || "User"}
              </p>
            </div>
            <p className="text-xs text-slate-500 truncate max-w-[220px] ml-4">
              {replyMessage.text || "[Media]"}
            </p>
          </div>
          <button
            onClick={() => setReplyMessage(null)}
            className="p-1 hover:bg-slate-100 rounded-full flex-shrink-0 text-slate-400 hover:text-slate-600"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
